# Synthesize (task: finalize)

Fills **blanks** in transformed parquet and **synthesizes missing days** (e.g. 5th when 4th and 6th exist). Writes new parquet files under `work_dir/final/`. Does **not** modify `work_dir/transformed/`.
...
**Output:** `work_dir/final/<band>/final_<YYYYMMDD>.parquet` for every date in the range (including missing days).  
Schema: same as transformed (`date`, `hour`, `band`, `freq_center_ghz`, `threshold_dbm`, `au_pct`); no missing cells.

**Run:** `task finalize` or `python -m synthesize.synthesize --work-dir work_dir`
